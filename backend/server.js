...
// NOUVEAU: Importation explicite des routeurs
const authRoutes = require('./routes/auth');     // Routes d'authentification
const userRoutes = require('./routes/users');    // Routes spécifiques aux utilisateurs
const adminRoutes = require('./routes/admin');   // Routes spécifiques aux administrateurs
...
// Routes
// IMPORTANT: Assurez-vous que les chemins ici correspondent aux appels du frontend
app.use('/api/auth', authRoutes);     // Les routes d'authentification seront préfixées par /api/auth
app.use('/api/users', userRoutes);    // Les routes utilisateurs seront préfixées par /api/users
app.use('/api/admin', adminRoutes);   // Les routes administrateurs seront préfixées par /api/admin
